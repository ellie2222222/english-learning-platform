...
import { UserLessonStatus, UserLessonStatusType } from "../enums/UserLessonStatus";
...
class UserLessonDto {
  ...
  private validateCurrentOrder = (currentOrder: number): void => {
    if (currentOrder !== undefined && (isNaN(currentOrder) || currentOrder < 0)) {
      throw new Error("Current order must be a number greater than or equal to 0");
    }
  };
  ...
  createUserLesson = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { userId, lessonId, currentOrder, status } = req.body;

      if (!userId || !lessonId) {
        throw new Error("Missing required fields: userId and lessonId are required");
      }
