...
import { UserCourseStatus, UserCourseStatusType } from "../enums/UserCourseStatus";
...
class UserCourseDto {
  ...
  private validateLessonFinished = (lessonFinished: number): void => {
    if (lessonFinished !== undefined && (isNaN(lessonFinished) || lessonFinished < 0)) {
      throw new Error("Lesson finished must be a number greater than or equal to 0");
    }
  };
  ...
  private validateAverageScore = (averageScore: number): void => {
    if (
      ...
      throw new Error("Average score must be a number between 0 and 100 or null");
    }
  };
  ...
  createUserCourse = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      const { userId, courseId, lessonFinished, averageScore, status } = req.body;

      if (!userId || !courseId) {
        throw new Error("Missing required fields: userId and courseId are required");
      }
